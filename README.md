...
- **Rock Paper Scissors Lizard Spock ChuckNorris**: A stupid extension of the Rock Paper Scissors Lizard Spock, invented by me in order to test the extensibility of the created structure. Actually Chuck Norris wins against everything and just draws against himself
...
Just as a brief summary of the structure, usually for small apps I prefer to use this kind of structure as I create separate folders for components, directives, controllers, services, etc...
...
I've been using **TDD** since a lot now. But since I discovered it, I fell in love with **BDD**, which is the approach I usually use at work and that I used in order to develop this simple application. It is a superset of TDD, the flow is still the same (designing tests, getting your tests failing, implementing the code, getting your tests passing) but you start the development from the e2e tests design/implementation, then you switch to the unit tests design/implementation, and at the end you switch to the real code design/implementation.

So now just a little note... **why I do prefer now BDD over TDD?**

The answer is in the difference between unit tests and e2e tests.
...
With the **E2E tests**, you can do something more, which is not (and should be not) covered by default from unit tests (expect of using some tricks or bad practices).

For example you can write a test for testing globally all the application, navigate to one page, interact wit the UI doing some action, going back to that page and seeing if the expected changes happened in the other page.
...
Therefore, you are sure that you will cover all the Business requirements of the user stories design e2e tests from the user stories description, and it allows you to be sure that you "limit" your code to the business requirements only, covering all of them, and avoiding an over-engineering of the code.

At the end, using **cucumber** (**gherkin engine**) you are able to write your feature files through plain English, so that the business can double check them and understand them, and ensure that you are covering correctly all the required functionalities, and the feature files could be also written by QA department in assistance with the business, and keep the developers focusing on the implementation of them.

Coming back to the development process, this is the flow I followed:

- Create a new branch for the feature you are going to develop
- Design the e2e tests describing all the functionalities and UI aspects you would expect from this feature
- Design the unit tests covering all the logic you are going to implement for designing the new feature
- Code the real implementation and check that it passes all the previous defined tests
...
Of course this process is iterative and incremental. This means that maybe during the unit tests, you can understand that you missed one aspect in the e2e tests, so switch back and add the new case. Or for example during the code implementation you understand that you need a new external file for this feature, so before to create the code, you switch back and you create the unit tests for the new files, and then you switch back to the code implementation.

## What if I want to change the rules of one of the games

You open the file `src/constants/game.constants.js` and there you can change the `label` if you want to display a new name, and you can change the `winsAgainst` and `losesAgainst` arrays if you want to change the rules, or simply change the term used in case of victory or of lose.
...
## What you were going to do more if you had few more time available
...
Here a list of possible issue you may encounter during the execution/exploration of the project.
